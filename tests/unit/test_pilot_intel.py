...
class TestComputeThreatLevel:
    ...
    def test_moderate_threat(self):
        assert (
            _compute_threat_level({"kills": 1500, "danger_ratio": 65, "solo_kills": 50})
            == "moderate"
        )

    def test_high_threat(self):
        assert (
            _compute_threat_level({"kills": 5500, "danger_ratio": 70, "solo_kills": 200}) == "high"
        )

    def test_extreme_threat(self):
        assert (
            _compute_threat_level({"kills": 15000, "danger_ratio": 85, "solo_kills": 600})
            == "extreme"
        )
    ...
    def test_danger_only_scoring(self):
        ...
        assert (
            _compute_threat_level({"kills": 0, "danger_ratio": 85, "solo_kills": 0}) == "moderate"
        )
