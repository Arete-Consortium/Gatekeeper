...
interface StreamingKillFeedProps {
  /** Filter kills to these system IDs */
  systemFilter?: number[];
  /** Filter kills to these region IDs */
  regionFilter?: number[];
  /** Minimum ISK value to display */
  minValue?: number;
  /** Include pod kills (default: true) */
  includePods?: boolean;
  /** Max kills to display */
  maxDisplay?: number;
  /** Title override */
  title?: string;
...
export function StreamingKillFeed({
  systemFilter = [],
  regionFilter = [],
  minValue = 0,
  includePods = true,
  ...
  compact = false,
}: StreamingKillFeedProps) {
  const { kills, isConnected } = useKillStream({
    maxKills: maxDisplay + 10,
    systemFilter,
    regionFilter,
    minValue,
    includePods,
  ...
  const recentKills = useMemo(() => kills.slice(0, maxDisplay), [kills, maxDisplay]);
      ...
      {recentKills.length > 0 ? (
        <div className="space-y-1 max-h-[600px] overflow-y-auto">
          {recentKills.map((kill: MapKill) => (
            <div
              key={kill.killId}
              className={`flex items-center justify-between ${rowPadding} rounded bg-card-hover/50 hover:bg-card-hover transition-colors text-sm`}
            >
